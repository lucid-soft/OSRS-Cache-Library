...
public class Group
{
     ...
     */
    public Group(int groupID, int name, int version, int crc32, byte[] digest, int[] filesIDS, int[] filesNames) {
    ...
    public void unload() {
        if (!isLoaded())
            throw new RuntimeException("Using nonloaded group.");
        packedFiles = null;
        for (int i = 0; i < files.length; i++)
            files[i].unload();
    }
    ...
    public ByteBuffer finish() {
        if (!isLoaded())
            throw new RuntimeException("Altering nonloaded group.");
    ...
    private ByteBuffer pack() {
        ...
        for (int i = 0,v = 0; i < files.length; i++, v += (files[i - 1].getData().getBuffer().length - v))
            packed.writeInt(files[i].getData().getBuffer().length - v);
    ...
    public void setVersion(int version) {
        ...
        if (this.version != version)
            this.indexChanged = this.needsRepack = true;
        this.version = version;
    }
